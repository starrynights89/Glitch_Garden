...
public class TextController : MonoBehaviour {
	...
	public Text text;
	private enum States {cell, cake, knife_0, manga_0, cell_cake, knife_1, manga_1, corridor_0};
	...
	void Update () {
		...
		else if (myState == States.cell_cake) 	{ cell_cake(); }
		else if (myState == States.knife_1) 	{ knife_1(); }
		else if (myState == States.manga_1) 	{ manga_1(); }
	...
	void cell_cake() {
		...
		else if (Input.GetKeyDown(KeyCode.T)) 	{ myState = States.corridor_0; }
	}
	...
	void manga_1() {
		text.text = "You ask the guard if he likes manga. He looks at you like you killed a " +
					"small animal. You decide to keep it to yourself.\n\n" +
					"Press R to return to your cell";
		if (Input.GetKeyDown(KeyCode.R)) 		{ myState = States.cell_cake; }
	}

	void corridor_0() {
		text.text = "You try the Ikea toolset on the lock to pry it open, but the lock " + 
					"doesn't budge. " + "The guard returns and notices the set. 'Hey! I " +
					"could use that set for assembling a PC from scratch. I'll trade you " +
					"my keys for that set.' You stare at him and without thinking too hard about " +
					"it pass the set over for a set of keys. The door slowly opens and you find " +
					"a corridor"; 
	}
}